...
#####################################################################
## PyTorch code reproduced to define scaled kaiming initialization ##
#####################################################################
...
def scaled_kaiming_normal_(tensor, a=0, mode='fan_in', nonlinearity='leaky_relu', weightscale=1.0):
    r"""Fills the input `Tensor` with values according to the method
    described in "Delving deep into rectifiers: Surpassing human-level
    performance on ImageNet classification" - He, K. et al. (2015), using a
    normal distribution, but with an additional scaling parameter 'weightscale'. 
    The resulting tensor will have values sampled from
    :math:`\mathcal{N}(0, weightscale*\text{std})` where
    ...
    std = weightscale*gain / math.sqrt(fan)
    with torch.no_grad():
        return tensor.normal_(0, std)


class initialize_scaled_kaiming(object):
    def __init__(self, weightscale, mode):
        self.weightscale = weightscale
        self.mode = mode

    def initialize(self, module):
        if isinstance(module, nn.Conv2d):
            scaled_kaiming_normal_(module.weight.data, mode=self.mode, nonlinearity='relu', weightscale=self.weightscale)
...
class Network(nn.Module):
    def __init__(self, config):
        ...
        init_function = initialize_scaled_kaiming(config['weightscale'], 'fan_out')
        self.apply(init_function.initialize)
